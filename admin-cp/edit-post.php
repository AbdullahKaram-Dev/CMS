                ...
                if ($imageError === 0) {
                    if ($imageSize <= 3000000) {
                        ...
                        if (move_uploaded_file($imageTmp, $image_Path)) {
                            ...
                            $insert = mysqli_query($connectToDB, "UPDATE `posts` SET `title` = '$title', `post` = '$post', `category` = '$category_Post', `image` = '$image_DB', `status` = '$status' WHERE `post_id` = '$_GET[post_id]' ");
                            if (isset($insert)) {

                                $message = '<div class="alert alert-success" role="alert"> تم تعديل المقالة بنجاح جارى تحويلك لصفحة المقالات </div>';
    ...
    <article class="col-lg-9">
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                $sql = mysqli_query($connectToDB,"SELECT * FROM `posts` WHERE `post_id` ='$_GET[post_id]'");
                ...
                <div class="panel panel-info">
                    <div class="panel-heading">
                        <b style="text-decoration-line: underline"> تعديل المقالة  :: <?php echo $POST_DATA['title']; ?></b>
                    </div>
                        ...
                        <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <textarea rows="8" class="form-control" name="post"
                                              id="post"><?php echo $POST_DATA['post']; ?></textarea>
                                </div>
                            </div>
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" id="category" name="category">
                                        ...
                                        while ($category = mysqli_fetch_assoc($categories)) {
                                            ...
                                            echo '<option value="' . $category['category_name'] . '" '.($POST_DATA['category'] == $category['category_name'] ? 'selected' : '').'>' . $category['category_name'] . '</option>';
                                        }
                                        ?>
                                    </select>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="status" id="status">
                                        <option value="published"<?php if ($POST_DATA['status'] == 'published'){ echo 'selected';}  ?>>مفعلة</option>
                                        <option value="unpublished"<?php if ($POST_DATA['status'] == 'unpublished'){ echo 'selected';}  ?>>غير مفعلة</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-sm-offset-2 col-sm-10">
                                    <button type="submit" name="submit" class="btn btn-info"> تحديث المقالة </button>
                                </div>
                            </div>
